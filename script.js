...
let currentView = 'year';         // 'month' | 'year' (по умолчанию годовой)
...
// Наблюдатели
let yearResizeObserver = null; // в этом варианте не используем масштабирование
...
function fitMonthRows() {
  ...
  // высота строки заголовков (Пн‑Вс)
  ...
  // сколько вертикального места доступно внутри .calendar
  const availH = calendarEl.clientHeight - headerH - rowGap;
  if (availH <= 0) return;

  // вычисляем высоту ячейки для 6 строк
  const plannedCellH = Math.floor((availH - rowGap * 5 - 2) / 6);

  // минимально комфортная высота, чтобы влезли цифра + подпись (8–12px)
  const MIN_COMFORT = 60;

  if (plannedCellH < MIN_COMFORT) {
    // Слишком низкие ячейки — НЕ задаём принудительные размеры, разрешаем скролл
  ...
  // Места достаточно — задаём высоты и можно не скроллить
  dayCells.forEach(cell => {
    cell.style.minHeight = plannedCellH + 'px';
    cell.style.height    = plannedCellH + 'px';
  });
  // calendarEl.style.overflowY = 'hidden'; // по желанию
}
...
function initCalendar() {
  ...
  updateScheduleButtonText();
  // если открыли во внешнем браузере с параметрами печати — обработать
  processPrintParams();
}
...
function setupEventListeners() {
  ...
  // Режимы/месяц‑год/переключение вида
...
function addDayTouchHandlers(el) {
  // Локальные переменные для жестов
  ...
  el.addEventListener('touchstart', (e) => {
    ...
    // Подготовка к возможному выделению диапазона (длинное удержание)
  ...
  el.addEventListener('touchmove', (e) => {
    ...
    if (selecting) {
      updateSelectionHighlight();
      e.preventDefault(); // не даём странице скроллиться во время “рисования”
    }
  }, { passive: false });
  ...
  const finish = (e) => {
    ...
    if (selecting) {
      ...
      // Короткий тап (без значимого движения) — открыть редактор дня
...
function setupSwipeNavigation() {
  ...
  const SWIPE_X = 50;  // порог по горизонтали (px)
  const SWIPE_Y = 30;  // допуск по вертикали (px)

  cal.addEventListener('touchstart', (e) => {
    if (currentView !== 'month' && currentView !== 'year') return;
    if (e.touches.length !== 1) return; // мультитач игнорируем
    swipeTracking = true;
    swipeConsumed = false;
    const t = e.touches[0];
  ...
  cal.addEventListener('touchmove', (e) => {
    ...
    // Горизонтальный жест — отменяем long‑press и перехватываем
...
// ========================
// Справка (обновлённый текст + про свайпы + про поделиться)
// ========================
function showHelp() {
  const modal = document.createElement('div');
  modal.style.cssText = `
  ...
  modal.innerHTML = `
      <div style="background: white; padding: 20px; border-radius: 10px; width: 90%; max-width: 500px; max-height: 80vh; overflow-y: auto;">
          ...
          <div style="margin-bottom: 20px;">
              ...
              <p><strong>Свайпы (телефон):</strong> смахните влево/вправо по календарю. Месячный — перелистывает месяцы, годовой — годы. Горизонтальный свайп отменяет долгое удержание.</p>
          </div>
...
// ========================
// Поделиться: Экспорт / Импорт / Печать (красивый вариант)
// ========================
...
function openShareModal() {
  ...
  // Печать (с fallback для Telegram WebView)
...
function tryPrint(kind /* 'month'|'year' */) {
  ...
  // 2) если Telegram WebView и печать не стартовала — fallback
  if (inTG) {
    setTimeout(() => { if (!printed) openExternalPrint(kind); }, 800);
  }
}

// Открыть во внешнем браузере с кодом и параметрами (режим/дата/что печатать)
function openExternalPrint(kind) {
  // Базовый код (режим + дата старта)
  const code = buildExportCode(false);
  const d = currentDate ? currentDate.toISOString().split('T')[0] : '';

  const url = new URL(location.href.split('#')[0]);
  url.searchParams.set('code', code);     // импорт базового графика
  url.searchParams.set('print', kind);    // 'month' | 'year'
  if (d) url.searchParams.set('d', d);    // текущая дата (месяц/год)

  const href = url.toString();

  try {
    if (isTelegramWebApp()) {
      Telegram.WebApp.openLink(href); // внешний браузер
    } else {
...
// Обработать параметры ?code & ?print & ?d при входе (для внешнего браузера)
function processPrintParams() {
  const params = new URLSearchParams(location.search);
  const code = params.get('code');
  const print = params.get('print'); // 'month' | 'year'
  ...
  if (print === 'month' || print === 'year') {
    ...
    updateViewButton();
    setTimeout(() => {
      if (print === 'month') ensureMonthThenPrint();
      else ensureYearThenPrint();
    }, 100);
  }
}
