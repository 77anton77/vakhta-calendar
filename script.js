...
function fitMonthRows() {
    ...
    // при желании можно спрятать скролл, но не обязательно
...
function initCalendar() {
    ...
    setupSwipeNavigation(); // ← добавьте эту строку
...
function setupSwipeNavigation() {
    ...
    cal.addEventListener('touchmove', (e) => {
        ...
        if (!swipeConsumed && Math.abs(dx) > SWIPE_X && Math.abs(dy) < SWIPE_Y) {
            // Отменяем потенциальный long‑press (массовое редактирование)
            if (longPressTimer) { clearTimeout(longPressTimer); longPressTimer = null; }
            selecting = false;
            swipeConsumed = true;
            e.preventDefault(); // блокируем прокрутку в момент свайпа
        }
    }, { passive: false });
    ...
    cal.addEventListener('touchend', (e) => {
        ...
        if (swipeConsumed) {
            // Определяем направление по финальному dx
...
function showHelp() {
    ...
    <p><strong>Свайпы (телефон):</strong> смахните влево/вправо по календарю.<br>
    — В месячном виде: перелистывание месяцев.<br>
    — В годовом виде: перелистывание годов.<br>
    Горизонтальный свайп отменяет долгое удержание, чтобы не мешать массовому редактированию.</p>
</div>
